...
//        GLuint TextureNumber = ::g_pTextureManager->getTextureIDFromName("Broc_tree_house.bmp");
    ...
    {
        GLuint TextureNumber = ::g_pTextureManager->getTextureIDFromName("Indoors01");
        GLuint textureUnit = 41;			// Texture unit go from 0 to 79
        glActiveTexture(textureUnit + GL_TEXTURE0);	// GL_TEXTURE0 = 33984
        glBindTexture(GL_TEXTURE_CUBE_MAP, TextureNumber);
        GLint cubeMap_01_LocID = glGetUniformLocation(shaderProgram, "cubeMap_02");
        glUniform1i(cubeMap_01_LocID, textureUnit);
    }
    // TODO: And so on, for the remaining cubemap texture assignments...
                ...
                cVAOManager* pVAOManager)
{
    ...
    if (pCurrentMesh->friendlyName == "holes")
         int db = -1;
    ...
    GLint bIsRigged_Location = glGetUniformLocation(program, "bIsRigged");
    if (pCurrentMesh->isRigged)
    {
        for (int i = 0; i < pCurrentMesh->v_Bones.size(); ++i)
        {
            std::string matrixName = "boneMatrices[";
            matrixName = matrixName + std::to_string(i);
            matrixName = matrixName + "]";
            GLint vBoneMatricies_Location = glGetUniformLocation(program, matrixName.c_str());
            glm::mat4 boneMatrix = pCurrentMesh->v_Bones[i]->GetModelMatrix();
            glUniformMatrix4fv(vBoneMatricies_Location, 1, GL_FALSE, glm::value_ptr(boneMatrix));
        }
        glUniform1f(bIsRigged_Location, (float)GL_TRUE);
    }
    else
    {
        glUniform1f(bIsRigged_Location, (float)GL_FALSE);

    }

    GLint bIsAssimpModel_Location = glGetUniformLocation(program, "bIsAssimpModel");

    glUniform1f(bIsAssimpModel_Location, (float)GL_FALSE);
...
//        if (gVAOManager.FindDrawInfoByModelName("Assembled_ISS.ply", modelInfo))
    ...
    if ( pVAOManager->FindDrawInfoByModelName( pCurrentMesh->meshName, modelInfo) )
    {
        ...
        if (pCurrentMesh->bInstanced)
        {
            for (int i = 0; i < pCurrentMesh->v_InstancedMatricies.size(); i++)
            {
                if (i > 99)
                    break;
                std::stringstream ssLight;
                ssLight << "instanceMatrices[" << i << "]";
                GLuint instanceLoc = glGetUniformLocation(program, ssLight.str().c_str());
                glUniformMatrix4fv(instanceLoc, 1, GL_FALSE, glm::value_ptr(pCurrentMesh->v_InstancedMatricies[i]));
            }
            GLuint isInstancedlLoc = glGetUniformLocation(program, "bIsInstanced");
            glUniform1f(isInstancedlLoc, GL_TRUE);
            glDrawElementsInstanced(GL_TRIANGLES, modelInfo.numberOfIndices, GL_UNSIGNED_INT, (void*)0, pCurrentMesh->mNumberOfInstances);
            glUniform1f(isInstancedlLoc, GL_FALSE);
